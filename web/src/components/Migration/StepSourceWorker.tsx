...
interface StepSourceWorkerProps {
  workers: Worker[];
  preselectedWorker?: Worker;
}

export function StepSourceWorker({ workers, preselectedWorker }: StepSourceWorkerProps) {
  const { state, setSourceWorker, nextStep } = useMigrationContext();
  const onlineWorkers = workers.filter((w) => w.online);

  useEffect(() => {
    if (preselectedWorker && !state.sourceWorker) {
      setSourceWorker(preselectedWorker);
    }
  }, [preselectedWorker, state.sourceWorker, setSourceWorker]);
  ...
  if (onlineWorkers.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-lg font-medium text-gray-900">No workers available</p>
        <p className="text-sm text-gray-500 mt-1">
          All workers must be online to be selected as a migration source.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      <div className="grid gap-3">
        {onlineWorkers.map((worker) => {
          const isSelected = state.sourceWorker?.id === worker.id;
